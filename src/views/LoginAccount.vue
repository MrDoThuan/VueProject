  ...
  <div class="container mid">
    ...
    <form @submit.prevent="login">
      <div class="form-group position-relative">
        <label for="username" class="title">Tên đăng nhập:</label>
        <div class="input-group">
          <input type="text" v-model="name" class="form-control" placeholder="Nhập tên đăng nhập">
      ...
      <div class="form-group position-relative">
        ...
        <div class="input-group">
          <input type="password" v-model="password" class="form-control" aria-describedby="helpId"
            placeholder="Nhập mật khẩu">
          <span class="input-group-text eye-icon"><i class="fa fa-eye"></i></span>
        </div>
        <a href=""><small id="helpId" class="text-muted title">Quên Mật Khẩu?</small></a>
      </div>
      ...
      <button type="submit" class="btn btn-danger btn-block title">Đăng Nhập</button>
    </form>
    ...
    login() {
      ...
      axios.get('http://localhost:3000/users')
        .then(response => {
          ...
          const users = response.data.find(users => users.name === this.name && users.password === this.password);
          if (users) {
